...
class PulseAudioSynthesizer {
   ...
   * Modulates pitch based on SpO2 when available, or uses neutral clinical tone
   * Timbre: Triangle wave with subtle downward pitch glide and exponential decay
   */
  public playBeat(spo2: number | null = null): void {
    ...
    try {
      ...
      // Pitch modulation: 540 Hz (low SpO2) to 740 Hz (100% SpO2), or neutral 640 Hz if SpO2 unavailable
      let baseFreq = 640;
      if (typeof spo2 === 'number' && !isNaN(spo2)) {
        const clampedSpo2 = Math.max(88, Math.min(100, spo2));
        baseFreq = 540 + (clampedSpo2 - 88) * 16.5;
      }
